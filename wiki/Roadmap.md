...
- **Constraint enforcement** — extend `UNIQUE` to non-rowid `PRIMARY KEY` and
  back it with index b-trees instead of a full scan (`UNIQUE` is already enforced
  on `INSERT` and `UPDATE`, by scan).
